# OpenSpace - Programmable AI Team OS（战术执行计划）
...
| [`docs/cursorsdkadapter.md`](docs/cursorsdkadapter.md) | 实现 | Cursor SDK 旁路调研 |
| [`docs/per-project-storage-design.md`](docs/per-project-storage-design.md) | 实现 | Per-project 存储设计 |
| [`docs/ui-reference/`](docs/ui-reference/README.md) | 视觉 | UI 基准规范 |
...
> **OpenSpace = Programmable AI Team OS** —— 你设定 Goal，AI 自动配备团队，团队自己设计 Workflow，系统持续沉淀经验并让每个 Agent 随项目成长。
...
- **数据目录**：`~/.openspace/openspace.db`；Agent cwd 取自 `project.workspace_path`，无 Agent 沙盒
...
| **Sprint 1** — Foundation + Goal → AI Team | Programmable AI Team OS 雏形 | ✅ 已交付 |
| **Sprint 2** — Workflow Framework | Workflow YAML + 3 内置模板 + 执行引擎 | ✅ 已交付 |
| **Sprint 3** — Responsibility + User Intervention | 批准流 + 用户介入 + Workflow Import/Export | ✅ 已交付 |
| **Sprint 4** — Delivery Loop (Scribe) | 沉淀 + Intelligence Tab | ✅ 已交付 |
| **Sprint 5** — Evolution Loop | Evaluator + Coach + Description 演化 | ✅ 已交付 |
| **Sprint 6** — Onboarding Loop + Skill Matrix | 新 Project 自动分析 + 能力地图 | ✅ 已交付 |
| **Sprint 7** — Team-First-Collaborative Workflow Design | Facilitator 主持的 Workflow Design Session | ✅ 已交付 |
...
1. 扫一遍 [`docs/product-brief.md`](docs/product-brief.md) 的核心定位，确认本 Sprint 没有偏离战略边界
2. 扫一遍 [`docs/technical-decisions.md`](docs/technical-decisions.md)，确认实现默认值和架构约束
3. 对涉及存储 / runtime / UI 的改动，分别同步 `per-project-storage-design` / `cursorsdkadapter` / `ui-reference`
...
三个 CLI 都是 spawn-per-message 模型；统一 `CLIAdapter` 接口已落地（`packages/server/src/agents/types.ts`）。

### Phase 0.5：UI 基准采集 ✅

视觉规范保留在 [`docs/ui-reference/`](docs/ui-reference/README.md)。

### v0 MVP ✅

按原 PLAN（v0.1 ~ v0.3）落地的"AI 编程协作室"基础版本，作为 Sprint 1 起点。能力包括：Monorepo 骨架、SQLite v0 schema、REST + WebSocket、Cursor Adapter、ContextBuilder、Message Router、前端 Shell。
...
- **S-1** `CursorSdkAdapter`：`@cursor/sdk` 直连派 adapter，与 `CursorAdapter`（spawn cursor-agent）并存；环境变量 `OPENSPACE_CURSOR_BACKEND=sdk|cli` 切换；`CLIAdapter.runDirect?` 钩子 + `runWithAdapter()` 统一 dispatcher
...
| **R-22** `.openspace/team.yaml` Project 级 Agent 定义 | git 可追踪 | 🟢 低 |
...
| **B-6** Tiled Live View（多 Agent 并排实时输出）| OpenSpace 相对 ClawTeam 的差异化机会 | 🟡 中 |
...
| **v1.1** | 2026-04-30 | 文档体系简化；v0 MVP 已交付清单折叠为一句话；当前焦点改为 Sprint 2 |
| v1.2 | 2026-04-30 | Sprint 1~7 全部交付，MVP 完成；新增 Sprint 4-ext（Cursor SDK Adapter 旁路）落地记录 |
| **v1.3** | 2026-05-12 | Sprint 8+ 章节重排（"广度优先" → "深度优先"），Sprint 8/9/10 主题改为 L4/L5/L6/L7 闭环打磨 |
| **v1.4** | 2026-05-29 | 清理过期 docs，只保留当前实现仍需引用的战略、技术决策、runtime、存储和 UI 文档 |
