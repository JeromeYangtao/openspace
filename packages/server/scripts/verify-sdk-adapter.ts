 ...
 *   OPENSPACE_CURSOR_BACKEND=sdk tsx packages/server/scripts/verify-sdk-adapter.ts
 *
 * 这个脚本不依赖 SQLite，只验证：
 *   1. createCursorAdapter() 在不同 OPENSPACE_CURSOR_BACKEND 下选对 adapter
...
} else if (rgConfig.reason && !rgConfig.reason.startsWith('OPENSPACE_CURSOR_BACKEND')) {
...
{
  const orig = process.env.OPENSPACE_CURSOR_BACKEND;

  delete process.env.OPENSPACE_CURSOR_BACKEND;
  const def = createCursorAdapter();
  expect(def instanceof CursorAdapter, '默认 → CursorAdapter');
  expect(def.name === 'cursor', '默认 name === cursor');

  process.env.OPENSPACE_CURSOR_BACKEND = 'cli';
  const cli = createCursorAdapter();
  expect(cli instanceof CursorAdapter, 'OPENSPACE_CURSOR_BACKEND=cli → CursorAdapter');

  process.env.OPENSPACE_CURSOR_BACKEND = 'sdk';
  const sdk = createCursorAdapter();
  expect(sdk instanceof CursorSdkAdapter, 'OPENSPACE_CURSOR_BACKEND=sdk → CursorSdkAdapter');
  ...
  if (orig === undefined) delete process.env.OPENSPACE_CURSOR_BACKEND;
