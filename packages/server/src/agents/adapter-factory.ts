 ...
 *
 * 统一构造点：根据环境变量 `OPENSPACE_CURSOR_BACKEND` 在 cursor-agent 子进程派 / @cursor/sdk
...
export function createCursorAdapter(): CLIAdapter {
  const backend = (process.env.OPENSPACE_CURSOR_BACKEND ?? 'cli').toLowerCase();
...
export interface SystemAdapterPick {
  ...
  /** cursor 与 codex 都未安装且未通过 OPENSPACE_SYSTEM_RUNTIME 显式指定时为 true */
  noRuntimeAvailable?: boolean;
}

export async function createSystemAdapter(): Promise<SystemAdapterPick> {
  const configured = (process.env.OPENSPACE_SYSTEM_RUNTIME ?? '').toLowerCase();
