...
export class CursorAdapter implements CLIAdapter {
  ...
  buildCommand(params: BuildCommandParams): SpawnSpec {
    ...
    if (params.thinking !== undefined && params.thinking !== null) {
      ...
      console.warn(
        '[cursor-cli] thinking is set but cursor-agent CLI does not support it; switch to SDK backend (OPENSPACE_CURSOR_BACKEND=sdk) to apply.',
      );
    }
    if (params.context) {
      // eslint-disable-next-line no-console
      console.warn(
        `[cursor-cli] context="${params.context}" is set but cursor-agent CLI does not support it; switch to SDK backend (OPENSPACE_CURSOR_BACKEND=sdk) to apply.`,
      );
    }
