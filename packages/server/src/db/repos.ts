 ...
 *
 * 设计原则：
 *   - 每个函数返回强类型对象（@openspace/shared 的 Channel / Agent / ChatMessage 等）
...
export const agentRunRepo = {
  ...
  start(
    ...
    const result = db
      .prepare(
        `INSERT INTO agent_runs (
           agent_id, channel_id, status, started_at, heartbeat_at, server_instance_id, ended_at, error_msg
         )
         VALUES (?, ?, ?, ?, ?, ?, NULL, NULL)`,
      )
      .run(
    ...
    return {
      ...
      started_at: ts,
      heartbeat_at: ts,
      server_instance_id: input.server_instance_id ?? null,
      ended_at: null,
      error_msg: null,
    };
  },
  ...
  /** 更新 run manager 心跳。 */
  touchHeartbeat(db: Database, id: number): void {
    db.prepare('UPDATE agent_runs SET heartbeat_at = ? WHERE id = ? AND ended_at IS NULL').run(
      now(),
      id,
    );
  },
  ...
  stop(db: Database, id: number, errorMsg?: string | null): void {
    const ts = now();
    db.prepare(
      'UPDATE agent_runs SET status = ?, ended_at = ?, error_msg = ? WHERE id = ? AND ended_at IS NULL',
    ).run('stopped', ts, errorMsg ?? null, id);
  },

  /** 列出所有活跃 run。 */
  listActive(db: Database): AgentRun[] {
    const rows = db
      .prepare(
...
// =============================================================================
// Agent Run Jobs — 轻量持久队列
// =============================================================================

export type AgentRunJobStatus = 'queued' | 'running' | 'done' | 'failed' | 'cancelled';
...
function rowToAgentRunJob(r: AgentRunJobRow): AgentRunJob {
  return {
    id: r.id,
    agent_id: r.agent_id,
    channel_id: r.channel_id,
    ...
    status: r.status as AgentRunJobStatus,
    error_msg: r.error_msg,
    created_at: r.created_at,
    started_at: r.started_at,
    ended_at: r.ended_at,
...
export const agentRunJobRepo = {
    ...
    const result = db
      .prepare(
        `INSERT INTO agent_run_jobs (
           agent_id, channel_id, trigger_message_id, parent_message_id, chain_depth,
           status, error_msg, created_at, started_at, ended_at
         )
  ...
  claimNext(db: Database): AgentRunJob | null {
    const row = db
      .prepare(
      ...
      .get() as AgentRunJobRow | undefined;
    if (!row) return null;
    const ts = now();
    const result = db
      .prepare(
        `UPDATE agent_run_jobs
         SET status = 'running', started_at = ?, error_msg = NULL
         WHERE id = ? AND status = 'queued'`,
      )
      .run(ts, row.id);
    if (result.changes === 0) return null;
    return { ...rowToAgentRunJob(row), status: 'running', started_at: ts };
  },

  complete(db: Database, id: number): void {
    const ts = now();
    db.prepare(
      `UPDATE agent_run_jobs
       SET status = 'done', ended_at = ?, error_msg = NULL
       WHERE id = ?`,
    ).run(ts, id);
  },

  fail(db: Database, id: number, errorMsg: string): void {
    const ts = now();
    db.prepare(
      `UPDATE agent_run_jobs
       SET status = 'failed', ended_at = ?, error_msg = ?
       WHERE id = ?`,
    ).run(ts, errorMsg, id);
  },

  cancelForChannel(db: Database, channelId: string, errorMsg: string): number {
    const ts = now();
    const result = db
      .prepare(
        `UPDATE agent_run_jobs
  ...
  countByStatus(db: Database, status: AgentRunJobStatus): number {
    const row = db
      .prepare('SELECT COUNT(*) AS count FROM agent_run_jobs WHERE status = ?')
...
// =============================================================================
// Runtime Sessions — Codex thread/session 复用
// =============================================================================

export const runtimeSessionRepo = {
  get(
    db: Database,
    input: { runtime: string; agent_id: string; channel_id: string },
  ): string | null {
    const row = db
      .prepare(
  ...
  upsert(
    db: Database,
    input: { runtime: string; agent_id: string; channel_id: string; session_id: string },
  ): void {
    const ts = now();
    db.prepare(
      `INSERT INTO runtime_sessions (
         runtime, agent_id, channel_id, session_id, created_at, updated_at
       )
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(runtime, agent_id, channel_id)
       DO UPDATE SET session_id = excluded.session_id, updated_at = excluded.updated_at`,
    ).run(input.runtime, input.agent_id, input.channel_id, input.session_id, ts, ts);
  },
};
