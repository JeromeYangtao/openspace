-- OpenSpace per-project SQLite schema
-- 对齐 docs/per-project-storage-design.md (D-21~D-25)
--
-- ★ 重要 ★：本 schema 是 per-project 版本。每个 <workspace>/.openspace/openspace.db 都用这套 schema。
-- 项目元数据（id / name / display_name / goal / team_rules / color / created_at / workspace_path）
-- 不在 db 里 —— 它们在 <workspace>/.openspace/project.json 中。
