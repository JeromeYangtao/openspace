...
// 否则像 OPENSPACE_CURSOR_BACKEND / CURSOR_API_KEY 这类后端切换变量在 import 时被读到的是 undefined。
//
// 优先级（后到不覆盖前到的）：
//   1. shell export 的 env (loadDotenv 不动)
//   2. .env (loadDotenv 注入)
//   3. ~/.openspace/settings.json (mergeUserSettings 注入)  ← UI 改这个
//   4. defaults
...
async function main() {
  ...
  } else if (__rgConfig.reason && !__rgConfig.reason.startsWith('OPENSPACE_CURSOR_BACKEND')) {
  ...
  if (projectsService.list().length === 0) {
    try {
      const result = projectsService.ensureDefaultWorkspace(config.defaultWorkspace);
      app.log.info(
        {
          workspace_path: result.project.workspace_path,
          is_new: result.isNew,
        },
        '[startup] default workspace opened',
      );
    } catch (e) {
      app.log.warn(
        { err: e, workspace_path: config.defaultWorkspace },
        '[startup] failed to open default workspace',
      );
    }
  }

  // Warm-up：把 ~/.openspace/projects.json 中所有 recent project 的 db 都打开
  ...
  const recoveredRuns = recoverInterruptedAgentRuns({ logger: app.log });
  if (recoveredRuns.stopped > 0) {
    app.log.info(recoveredRuns, '[startup] stopped interrupted agent runs');
  }
  ...
  app.get('/api/health', async () => ({
    ok: true,
    version: '0.0.1',
    openspace_home: config.openspaceHome,
  ...
  await registerStaticWeb(app, { logger: app.log });
  ...
  try {
    ...
    app.log.info(`✓ OpenSpace server listening on http://${config.host}:${config.port}`);
