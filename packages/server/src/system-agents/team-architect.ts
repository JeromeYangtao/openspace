...
import { TEAM_ARCHITECT_TIMEOUT_MS } from '@openspace/shared';
import type { ReasoningEffort, Runtime, TeamSuggestion, TeamSuggestionAgent } from '@openspace/shared';
...
export async function suggestTeam(input: SuggestTeamInput): Promise<TeamSuggestion> {
  ...
  if (noRuntimeAvailable) {
    ...
    return fallbackTeam(
      defaultRuntime,
      'no coding runtime available (install cursor-agent or codex CLI, or set OPENSPACE_SYSTEM_RUNTIME)',
    );
  }
...
function buildTeamArchitectPrompt(input: SuggestTeamInput, defaultRuntime: Runtime): string {
  ...
  return [
    'You are the Team Architect for a new AI engineering team in OpenSpace (a local AI Team OS).',
    '',
 ...
 */
function parseContext(raw: unknown): import('@openspace/shared').ContextSize | null {
  if (raw === null || raw === undefined) return null;
