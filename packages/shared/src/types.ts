...
export interface RuntimeDetection {
  ...
  path?: string;
  note?: string;
  /** MVP 当前 OpenSpace 端是否已实装该 runtime（未实装会 disable） */
  enabled_in_openspace: boolean;
}
...
export interface Workflow {
  ...
  /** 'builtin'（随 OpenSpace 发行）或 'user'（用户自建 / Facilitator 产出） */
...
//
// 用户在 UI Settings 页配置的 Cursor backend 选项，持久化到 ~/.openspace/settings.json。
