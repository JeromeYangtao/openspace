...
export function OpenProjectDialog({ open, onClose }: Props) {
  ...
  const handleOpen = async () => {
    ...
    try {
      ...
      // POST /api/projects/open：服务端检测 <path>/.openspace/ 是否已存在 → reopen 或新建
      ...
      if (isNew) {
        ...
        // Reopen：channel 已经在 .openspace/openspace.db 中；仅刷新 store
        try {
      ...
      <div className="bg-bg-card border-2 border-black rounded-xl shadow-[6px_6px_0_0_#000] w-full max-w-xl">
        ...
        <div className="p-5 space-y-4">
          <div className="text-sm text-text-secondary">
            Pick a code folder. OpenSpace will create a Project bound to it and a default
