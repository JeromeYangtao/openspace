...
import type { Agent, ChatMessage, MessageMetadata } from '@openspace/shared';
import { getChannelAgents, getChannelMessages } from '../lib/api';
...
export function ThreadPanel({ channelId }: Props) {
  ...
  useEffect(() => {
    ...
    void getChannelAgents(channelId)
      .then((rows) => {
        if (!cancelled) setChannelAgents(rows);
      })
      .catch(() => {
        if (!cancelled) setChannelAgents([]);
      });
    return () => {
      cancelled = true;
    };
