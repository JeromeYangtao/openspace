...
import type { CursorBackendStatus, RuntimeDetection } from '@openspace/shared';
...
export function WelcomePage() {
  ...
  const codexReady = Boolean(codex?.installed && codex?.enabled_in_openspace);
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8 overflow-y-auto">
      <div className="max-w-xl w-full bg-bg-card border-2 border-black rounded-xl p-8 shadow-[6px_6px_0_0_#000]">
        <h1 className="text-3xl font-bold mb-2">Welcome to OpenSpace</h1>
        ...
        {!codingRuntimeReady && (
          <div className="mb-6 p-4 bg-accent-yellow border-2 border-black rounded">
            ...
            <div className="text-sm space-y-1.5">
              <div>选一种方式让 OpenSpace 调用本地 coding agent：</div>
        ...
        <div className="space-y-4">
          ...
          <div className="text-[12px] font-mono text-text-muted">
            Pick any local code folder. OpenSpace creates a workspace + default <span className="font-mono">#general</span> channel
        ...
        <div className="mt-6 text-xs text-text-muted font-mono">
          OpenSpace v1.0 · local-only · no login · programmable AI team OS
        </div>
      </div>
